...
const SinglePost = ({ item, authUser }) => {
    ...
    const handleReplySubmit = async (commentId) => {
        try {
            ...
            const response = await axios.post(`http://localhost:4000/api/v1/posts/${item._id}/comments/${commentId}/reply`, {
                userId: authUser?._id,
                text: replyContent
            });
    ...
    const handleMainCommentSubmit = async (postId) => {
        try {
            axios.defaults.withCredentials = true;
            const response = await axios.post(`http://localhost:4000/api/v1/comment/createCommentInGameGroup`, {
