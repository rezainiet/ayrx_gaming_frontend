    ...
    const handleClickLike = async () => {
        try {
            if (liked) {
                ...
                const response = await axios.delete(`http://localhost:4000/api/forumPosts/${post._id}/like/${authUser._id}`);
                console.log(response.data);
                // Update liked state to false
                setLiked(false);
            } else {
                // User has not liked the post, so add their like
                const response = await axios.post(`http://localhost:4000/api/forumPosts/${post._id}/like`, {
                    userId: authUser._id
                });
                console.log(response.data);
